...
bool nextGenerationTest()
{
	...
	string methodType = "triangles";

	readAllFingerprints(database);
	makeAllVaults(methodType, 1.0);
...
bool readAllFingerprintsTest()
{
	...
	for(int dbYear = 2002; dbYear<=2006; dbYear+=2)
	{
		for(int dbNum = 1; dbNum<=4; dbNum++)
		{
			...
			try
			{
				...
				vector<vector<vector<minutia> > > goodFingerprints = allFingerprints;
	...
	try //bad DB name
	{
		...
		vector<vector<vector<minutia> > > badFingerprints = allFingerprints;
